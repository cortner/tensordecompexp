"""2D Chebyshev Tucker model optimized with BFGS."""

# bfgs_tucker_chebyshev_2d_matrixC.py
...
if __name__ == "__main__":
    ...
    for idx, C in enumerate(C_matrices):
        C_key = f"C_case_{idx}"
        all_results[C_key] = {}
        for R in range(1, 21):  # Rank sweep (1,1) → (20,20)
            R_x = R_y = R
            print(f"\n=== Running BFGS Tucker experiment for C={idx}, R_x={R_x}, R_y={R_y} ===")
    ...
    with open("bfgs_tucker_rank_sweep_results.json", "w") as f_out:
        json.dump(all_results, f_out, indent=4)

    print("\nAll Tucker BFGS runs complete. Results saved to bfgs_tucker_rank_sweep_results.json")
