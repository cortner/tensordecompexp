"""Adam optimization for 2D Chebyshev Tucker; optional TensorLy Tucker or random initialization."""
...
if __name__ == "__main__":
    ...
    for idx, C in enumerate(C_matrices):
        ...
        for R in range(1, 21):  # Rank sweep 1 → 20
            ...
            metrics, coeffs, config, savedir = run_adam_experiment(
                ...
                R_y=R_y,
                C=C,
                init_mode="tucker",  # <-- warm-start using Tucker decomposition
            )
    ...
    with open("adam_tucker_init_rank_sweep_results.json", "w") as f_out:
        json.dump(all_results, f_out, indent=4)

    print("\nAll Tucker runs complete. Results saved to adam_tucker_init_rank_sweep_results.json")