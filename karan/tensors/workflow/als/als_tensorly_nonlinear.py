"""TensorLy Tucker + Chebyshev lift for a nonlinear radial-angular 2D test function."""
...
# ============================================================
def f(x, y, C=None, gamma=4.0):
    """
    Smooth physical-style test function modeling a radial distribution
    multiplied by an angular (spherical-harmonic-like) term.

    f(x,y) = exp(-gamma * r^2) * (1 + cos(theta))
    where r = sqrt(u^2 + v^2), theta = atan2(v, u),
    and (u, v) = C @ (x, y).
    """
    if C is None:
        C = np.eye(2)

    # Apply anisotropic linear transform
    coords = np.stack([x.ravel(), y.ravel()], axis=1)
    u, v = (coords @ C.T).T

    # Polar coordinates in transformed space
    r = np.sqrt(u**2 + v**2)
    theta = np.arctan2(v, u)

    # Radial x angular function
    vals = np.exp(-gamma * r**2) * (1.0 + np.cos(theta))
...
if __name__ == "__main__":
    ...
    with open("als_tucker_tensorly_sin.json", "w") as f_out:
        json.dump(all_results, f_out, indent=4)

    print("\nAll Tucker runs complete. Results saved to als_tucker_tensorly_sin.json")