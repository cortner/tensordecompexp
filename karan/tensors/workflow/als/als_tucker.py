"""Iterative 2D ALS Tucker: F approx (T_x A) G (T_y B)^T with alternating mode updates."""
...
def run_als_tucker_experiment(
    ...
    for it in range(n_iter):
        ...
        try:
            ...
            # Handle rare SVD failure gracefully
            print(f"Iter {it+1}: SVD did not converge in A update — retrying with small damping.")
        ...
        try:
            ...
            print(f"Iter {it+1}: SVD did not converge in B update — retrying with small damping.")
...
if __name__ == "__main__":
    ...
    for idx, C in enumerate(C_matrices):
        ...
        for R in range(1, 21):  # sweep Tucker ranks 1–10
    ...
    with open("als_tucker_rank_sweep_results.json", "w") as f_out:
        json.dump(all_results, f_out, indent=4)

    print("\nAll ALS–Tucker runs complete. Results saved to als_tucker_rank_sweep_results.json")
