"""Standard 2D rank-1 ALS CP on a Chebyshev grid with optional lambda weights and experiment outputs."""
...
def compute_reference_coeffs(d_x, d_y, C):
    ...
    coeffs = coeffs_flat.reshape((d_x+1, d_y+1))
    return coeffs
...
def run_als_experiment(
    ...
    if use_lambda:
        ...
        lambdas = np.ones(R)  # but won’t be updated
...
if __name__ == "__main__":
    ...
    for idx, C in enumerate(C_matrices):
        ...
        for R in range(1, 21):  # sweep ranks 1 → 20
            ...
            metrics, coeffs, config, savedir = run_als_experiment(
                ...
                random_seed=42,
                outdir="als_results",
                use_lambda=True,  # or True depending on what you want
            )
    ...
    with open("als_true_eps_rank_sweep_results.json", "w") as f_out:
        json.dump(all_results, f_out, indent=4)

    print("\nAll ALS runs complete. Results saved to als_true_eps_rank_sweep_results.json")
