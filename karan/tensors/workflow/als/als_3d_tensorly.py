"""3D TensorLy Tucker decomposition on F, then least-squares lift of factors into the Chebyshev basis."""

# now we would talk about same spaces here
# value space and coefficient space
# by including the weights so the system is orthogonal
# which makes minimising the function equivalent to minimising the tensor

#  ======================================
#  3D SIMPLE TUCKER + CHEBYSHEV CONVERSION
#  ======================================

import numpy as np
import matplotlib.pyplot as plt
from matplotlib import cm
from numpy.polynomial.chebyshev import chebvander
import json
import os
from datetime import datetime

import tensorly as tl
from tensorly.decomposition import tucker as tl_tucker
from tensorly.tenalg import multi_mode_dot

tl.set_backend("numpy")


# ============================================================
# 3D Function to Approximate (Matrix C: 3x3)
# ============================================================
def f3d(x, y, z, C=None):
    """
    Generalized 3D test function with anisotropy via a 3x3 matrix C.

        f(x,y,z) = 1 / (1 + || C [x, y, z]^T ||^2)

    x, y, z: broadcastable arrays (same shape)
    C: 3x3 numpy array (anisotropy matrix)
    """
    if C is None:
        C = np.eye(3) * 5.0

    coords = np.stack([x.ravel(), y.ravel(), z.ravel()], axis=1)  # (Npts, 3)
    trans = coords @ C.T                                          # (Npts, 3)
    vals = 1.0 / (1.0 + np.sum(trans**2, axis=1))
    return vals.reshape(x.shape)


# ============================================================
# Node Generators
# ============================================================
def generate_nodes(num_points, mode="chebyshev"):
    """Generate 1D nodes in [-1,1]."""
    if mode == "chebyshev":
        return np.cos(np.pi * np.arange(num_points) / (num_points - 1))
    elif mode == "uniform":
        return np.linspace(-1, 1, num_points)
    elif mode == "random":
        return np.random.uniform(-1, 1, num_points)
    else:
        raise ValueError(f"Unknown mode {mode}")


# ============================================================
# 3D True Chebyshev coefficient tensor (interpolation)
# ============================================================
def compute_reference_coeffs_3d(d_x, d_y, d_z, C):
    """
    Compute the true 3D Chebyshev coefficients of f3d(x,y,z) by interpolation.
    Uses a (d_x+1) x (d_y+1) x (d_z+1) Chebyshev tensor grid.
    """
    # Chebyshev nodes for each dimension
    nodes_x = np.cos(np.pi * np.arange(d_x + 1) / d_x)
    nodes_y = np.cos(np.pi * np.arange(d_y + 1) / d_y)
    nodes_z = np.cos(np.pi * np.arange(d_z + 1) / d_z)

    X, Y, Z = np.meshgrid(nodes_x, nodes_y, nodes_z, indexing="ij")
    F = f3d(X, Y, Z, C=C)  # shape: (d_x+1, d_y+1, d_z+1)

    F_flat = F.ravel(order="F")

    Tx = chebvander(nodes_x, d_x)  # (d_x+1) x (d_x+1)
    Ty = chebvander(nodes_y, d_y)  # (d_y+1) x (d_y+1)
    Tz = chebvander(nodes_z, d_z)  # (d_z+1) x (d_z+1)

    # Design matrix: kron(Tz^T, kron(Ty^T, Tx^T))
    A = np.kron(Tz.T, np.kron(Ty.T, Tx.T))

    coeffs_flat, *_ = np.linalg.lstsq(A, F_flat, rcond=None)
    coeffs = coeffs_flat.reshape((d_x + 1, d_y + 1, d_z + 1), order="F")
    return coeffs


# ============================================================
# 3D TUCKER EXPERIMENT (NO ALS)
# ============================================================
def run_als_tucker_experiment_3d(
    N_x=64, N_y=64, N_z=64,
    d_x=31, d_y=31, d_z=31,
    R_x=5, R_y=5, R_z=5,
    n_iter=200,
    C=np.eye(3) * 5.0,
    resolution=50,
    num_test_points=2048,  # unused but kept for config compatibility
    epsilon=1e-6,
    random_seed=42,
    outdir="als_tucker3d_results",
    train_points="chebyshev",
    test_points="uniform",   # unused but kept for structural consistency
):
    """
    3D version:
    Runs Tucker decomposition on 3D function f3d(x,y,z).

    Steps:
      1. Sample F on a 3D grid of (N_x, N_y, N_z) points.
      2. Run TensorLy Tucker with rank [R_x, R_y, R_z].
      3. Fit each Tucker factor in Chebyshev basis (LS).
      4. Form Chebyshev coefficient tensor of shape (d_x+1, d_y+1, d_z+1).
      5. Compare with "true" interpolation coefficients on a Chebyshev grid.
      6. Compute the SAME metrics as in the 2D version:

         - final_train_weighted_rmse  (here: RMSE on training 3D grid)
         - rmse_eval_grid            (RMSE on dense evaluation 3D grid)
         - l2_norm_error_train       (||F - F_reconstructed||_F)
         - l2_coeff_error            (||C_interp - C_tucker||_F)
         - rel_l2_coeff_error        (normalized version)
    """

    base_dir = os.path.dirname(os.path.abspath(__file__))
    results_root = os.path.join(base_dir, outdir)
    np.random.seed(random_seed)

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    outdir = os.path.join(results_root, f"tucker3d_{timestamp}")
    os.makedirs(outdir, exist_ok=True)

    # -------------------- Training Setup --------------------
    x_nodes = generate_nodes(N_x, train_points)
    y_nodes = generate_nodes(N_y, train_points)
    z_nodes = generate_nodes(N_z, train_points)

    X, Y, Z = np.meshgrid(x_nodes, y_nodes, z_nodes, indexing='ij')
    F = f3d(X, Y, Z, C=C)  # training tensor (N_x × N_y × N_z)

    # -------------------- TensorLy Tucker (3D) --------------------
    print("Running 3D TensorLy Tucker...")
    tucker_tensor, rec_errors = tl_tucker(
        F,
        rank=[R_x, R_y, R_z],
        n_iter_max=n_iter,
        init="svd",
        tol=1e-8,
        random_state=random_seed,
        return_errors=True,
    )

    core_val, factors = tucker_tensor
    Ux, Uy, Uz = factors  # shapes: (N_x x R_x), (N_y x R_y), (N_z x R_z)

    F_tucker_val = tl.tucker_to_tensor(tucker_tensor)
    tucker_rmse = np.sqrt(np.mean((F - F_tucker_val) ** 2))
    print(f"3D value-space Tucker RMSE (before Chebyshev lift): {tucker_rmse:.3e}")

    # -------------------- Fit Tucker factors in Chebyshev basis --------------------
    Tx = chebvander(x_nodes, d_x)  # N_x × (d_x+1)
    Ty = chebvander(y_nodes, d_y)  # N_y × (d_y+1)
    Tz = chebvander(z_nodes, d_z)  # N_z × (d_z+1)

    A_x, *_ = np.linalg.lstsq(Tx, Ux, rcond=None)  # (d_x+1) × R_x
    A_y, *_ = np.linalg.lstsq(Ty, Uy, rcond=None)  # (d_y+1) × R_y
    A_z, *_ = np.linalg.lstsq(Tz, Uz, rcond=None)  # (d_z+1) × R_z

    # -------------------- Build 3D Chebyshev coefficient tensor --------------------
    # C_tucker = G ×_0 A_x ×_1 A_y ×_2 A_z
    C_tucker = multi_mode_dot(core_val, [A_x, A_y, A_z], modes=[0, 1, 2])

    # Reconstruction using Chebyshev representation on the training grid:
    Ux_val = Tx @ A_x  # (N_x × R_x)
    Uy_val = Ty @ A_y  # (N_y × R_y)
    Uz_val = Tz @ A_z  # (N_z × R_z)

    F_reconstructed = tl.tucker_to_tensor((core_val, [Ux_val, Uy_val, Uz_val]))
    diff_final = F - F_reconstructed

    # EXACT SAME METRICS NAMES AS 2D VERSION:
    # final_train_weighted_rmse := RMSE on training grid (no weights here)
    rmse_train = np.sqrt(np.mean(diff_final**2))
    l2_norm_error = np.linalg.norm(diff_final)

    # -------------------- Dense evaluation grid (for rmse_eval_grid) --------------------
    x_eval = np.linspace(-1, 1, resolution)
    y_eval = np.linspace(-1, 1, resolution)
    z_eval = np.linspace(-1, 1, resolution)

    X_eval, Y_eval, Z_eval = np.meshgrid(x_eval, y_eval, z_eval, indexing='ij')
    F_true_eval = f3d(X_eval, Y_eval, Z_eval, C=C)

    Tx_eval = chebvander(x_eval, d_x)  # resolution × (d_x+1)
    Ty_eval = chebvander(y_eval, d_y)
    Tz_eval = chebvander(z_eval, d_z)

    Ux_eval = Tx_eval @ A_x   # (resolution × R_x)
    Uy_eval = Ty_eval @ A_y   # (resolution × R_y)
    Uz_eval = Tz_eval @ A_z   # (resolution × R_z)

    F_pred_eval = tl.tucker_to_tensor((core_val, [Ux_eval, Uy_eval, Uz_eval]))
    rmse_grid = np.sqrt(np.mean((F_true_eval - F_pred_eval) ** 2))

    # -------------------- Compare with true Chebyshev coeffs --------------------
    C_interp = compute_reference_coeffs_3d(d_x, d_y, d_z, C)

    l2_coeff_error = np.linalg.norm(C_interp - C_tucker)
    rel_l2_coeff_error = l2_coeff_error / np.linalg.norm(C_interp)

    # -------------------- METRICS (SAME KEYS AS 2D) --------------------
    metrics = {
        "final_train_weighted_rmse": float(rmse_train),
        "rmse_eval_grid": float(rmse_grid),
        "l2_norm_error_train": float(l2_norm_error),
        "l2_coeff_error": float(l2_coeff_error),
        "rel_l2_coeff_error": float(rel_l2_coeff_error),
    }

    # -------------------- Save Results --------------------
    with open(os.path.join(outdir, "metrics.json"), "w") as f_out:
        json.dump(metrics, f_out, indent=4)

    np.savez(
        os.path.join(outdir, "coeffs3d.npz"),
        A_x=A_x, A_y=A_y, A_z=A_z,
        G=core_val,
        C_tucker=C_tucker,
        Ux=Ux, Uy=Uy, Uz=Uz,
    )

    config = {
        "N_x": N_x, "N_y": N_y, "N_z": N_z,
        "d_x": d_x, "d_y": d_y, "d_z": d_z,
        "R_x": R_x, "R_y": R_y, "R_z": R_z,
        "n_iter": n_iter,
        "C": C.tolist(),
        "resolution": resolution,
        "num_test_points": num_test_points,
        "epsilon": epsilon,
        "random_seed": random_seed,
        "timestamp": timestamp,
        "train_points": train_points,
        "test_points": test_points,
    }

    with open(os.path.join(outdir, "config.json"), "w") as f_out:
        json.dump(config, f_out, indent=4)

    # -------------------- Plot: mid-z slice as 2D surfaces --------------------
    mid_k = N_z // 2
    X_slice = X[:, :, mid_k]
    Y_slice = Y[:, :, mid_k]
    F_slice = F[:, :, mid_k]
    F_rec_slice = F_reconstructed[:, :, mid_k]

    fig = plt.figure(figsize=(12, 5))

    ax1 = fig.add_subplot(121, projection="3d")
    ax1.plot_surface(X_slice, Y_slice, F_slice, cmap=cm.viridis)
    ax1.set_title(f'Original f3d(x,y,z_mid) ({train_points} training)')

    ax2 = fig.add_subplot(122, projection="3d")
    ax2.plot_surface(X_slice, Y_slice, F_rec_slice, cmap=cm.viridis)
    ax2.set_title(f'Reconstructed (R=({R_x},{R_y},{R_z}))')

    plt.tight_layout()
    plt.savefig(os.path.join(outdir, "reconstruction_mid_slice.png"))
    plt.close(fig)

    return metrics, {
        "A_x": A_x, "A_y": A_y, "A_z": A_z, "G": core_val
    }, config, outdir


# ============================================================
# Rank-Sweep Example (3D)
# ============================================================
if __name__ == "__main__":
    def rotation_matrix_3d_z(theta_deg):
        """Simple rotation about z-axis for anisotropy examples."""
        theta = np.deg2rad(theta_deg)
        return np.array([
            [np.cos(theta), -np.sin(theta), 0.0],
            [np.sin(theta),  np.cos(theta), 0.0],
            [0.0,            0.0,          1.0],
        ])

    # A few different 3x3 anisotropy matrices
    C_matrices = [
        np.diag([5.0, 1.0, 1.0]),
        np.diag([1.0, 5.0, 1.0]),
        np.diag([1.0, 1.0, 5.0]),
        rotation_matrix_3d_z(30) @ np.diag([5.0, 1.0, 1.0]),
        5.0 * np.eye(3),
    ]

    all_results = {}

    for idx, C in enumerate(C_matrices):
        C_key = f"C_case_{idx}"
        all_results[C_key] = {}

        # you can tweak N_x, N_y, N_z and degrees to keep computations reasonable
        for R in range(1, 21):  # sweep Tucker ranks
            print(f"\n=== 3D Tucker experiment for C_case={idx}, Rank={R} ===")
            metrics, coeffs, config, savedir = run_als_tucker_experiment_3d(
                N_x=32, N_y=32, N_z=32,    # training grid sizes (adjust as you like)
                d_x=31, d_y=31, d_z=31,   # Chebyshev degrees
                R_x=R, R_y=R, R_z=R,
                n_iter=100,
                C=C,
                train_points="chebyshev",   # or "uniform" / "random"
                test_points="uniform",
                num_test_points=2048,
                resolution=40,            # dense eval grid per dimension
                random_seed=42,
                outdir="als_tucker3d_cheb_results",
            )

            all_results[C_key][f"rank_{R}"] = {
                "metrics": metrics,
                "config": config,
                "savedir": savedir,
            }

    with open("als_tucker3d_tensorly_cheb.json", "w") as f_out:
        json.dump(all_results, f_out, indent=4)

    print("\nAll 3D Tucker runs complete. Results saved to als_tucker3d_tensorly_cheb.json")
