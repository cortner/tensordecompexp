"""2D ALS CP experiment runner and rank sweep (lambda on/off); no interp-SVD init."""
...
# ============================================================
# ALS Training Function (no interp_svd init)
# ============================================================
def run_als_experiment(
    ...
    If use_lambda=False:
      lambdas are fixed to 1 and are not updated.
    """
    # Base dir for outputs (robust for notebooks as well)
    try:
    ...
    for it in range(n_iter):
        ...
        # ----- Update A_coeffs (component-wise) using lstsq -----
        for r in range(R):
            ...
            target = rhs_eval / (scale * den)         # length-N vector

            a, *_ = np.linalg.lstsq(Tx, target, rcond=None)

            # Normalize in evaluation space and fold into lambda
            if use_lambda:
                Acol = Tx @ a
                nA = np.linalg.norm(Acol) + 1e-12
                if nA > 0:
                    a /= nA
                    lambdas[r] *= nA
            A_coeffs[:, r] = a

        # ----- Update B_coeffs (component-wise) using lstsq -----
        for r in range(R):
            Fr = F - sum(
                lambdas[k] * np.outer(Tx @ A_coeffs[:, k], Ty @ B_coeffs[:, k])
                for k in range(R) if k != r
            ...
            den = float(Z @ Z) + 1e-12
            scale = lambdas[r] if use_lambda else 1.0
            target = rhs_eval / (scale * den)         # length-M vector

            b, *_ = np.linalg.lstsq(Ty, target, rcond=None)

            if use_lambda:
                Bcol = Ty @ b
                nB = np.linalg.norm(Bcol) + 1e-12
                if nB > 0:
                    b /= nB
                    lambdas[r] *= nB
            B_coeffs[:, r] = b

        # ----- Update lambdas (joint small solve) -----
        if use_lambda:
            A_eval = Tx @ A_coeffs
            B_eval = Ty @ B_coeffs
            ...
            lambdas = np.linalg.solve(G + 1e-12 * np.eye(R), bb)
            # mild clipping prevents drift
            # lambdas = np.clip(lambdas, 1e-8, 1e8)
...
if __name__ == "__main__":
    ...
    with open("als_false_eps_rank_sweep_results.json", "w") as f_out:
        json.dump(all_results, f_out, indent=4)

    print("\nAll ALS runs complete. Results saved to als_false_eps_rank_sweep_results.json")
