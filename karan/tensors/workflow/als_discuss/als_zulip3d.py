"""3D Christoph-style Tucker ALS mirroring the 2D Zulip algorithm."""

# this is a new type of algorithm proposed by Christoph for 3d
# dated 2025-12-06

#  ======================================
#  3D SIMPLE TUCKER + CHEBYSHEV (Christoph-style ALS)
#  ======================================
...
def run_als_tucker_experiment_3d(
    ...
    for it in range(n_iter):
        ...
        # =====================================================
        # YZ features on the grid from current A_y, A_z
        Uy = Ty @ A_y  # (N_y, R_y)
        Uz = Tz @ A_z  # (N_z, R_z)

        # Combined feature matrix for (y,z): shape (N_y*N_z, R_y*R_z)
        # Use np.kron(Uz, Uy) so that columns correspond to (b,c) with b fastest,
        # matching reshape(..., order="F")
        YZ_feat = np.kron(Uz, Uy)

        # Flatten F with x as rows, (y,z) as columns
        F_flat_x = F.reshape(N_x, N_y * N_z, order="F")

        # Design matrix: vec(F_flat_x) = (YZ_feat ⊗ Tx) vec(tildeG_x)
        D_A = np.kron(YZ_feat, Tx)           # (N_x*N_y*N_z, (d_x+1)*R_y*R_z)
        target_A = F_flat_x.reshape(-1, order="F")

        try:
            tildeGx_vec, *_ = np.linalg.lstsq(D_A, target_A, rcond=1e-12)
        except np.linalg.LinAlgError:
            print("Stopping early due to LinAlgError in lstsq.")
            break

        tildeGx = tildeGx_vec.reshape((d_x + 1, R_y * R_z), order="F")
        ...
        # =====================================================
        # Recompute Ux, Uz with updated A_x; A_y, A_z still old
        Ux = Tx @ A_x
        Uz = Tz @ A_z
        ...
        D_B = np.kron(XZ_feat, Ty)                          # (N_x*N_y*N_z, (d_y+1)*R_x*R_z)
        target_B = F_flat_y.reshape(-1, order="F")

        try:
            tildeGy_vec, *_ = np.linalg.lstsq(D_B, target_B, rcond=1e-12)
        except np.linalg.LinAlgError:
            print("Stopping early due to LinAlgError in lstsq.")
            break
        tildeGy = tildeGy_vec.reshape((d_y + 1, R_x * R_z), order="F")
        ...
        D_C = np.kron(XY_feat, Tz)                          # (N_x*N_y*N_z, (d_z+1)*R_x*R_y)
        target_C = F_flat_z.reshape(-1, order="F")

        try:
            tildeGz_vec, *_ = np.linalg.lstsq(D_C, target_C, rcond=1e-12)
        except np.linalg.LinAlgError:
            print("Stopping early due to LinAlgError in lstsq.")
            break
        tildeGz = tildeGz_vec.reshape((d_z + 1, R_x * R_y), order="F")
...
if __name__ == "__main__":
    ...
    for idx, C in enumerate(C_matrices):
        ...
        for R in range(1, 21):
            ...
            metrics, coeffs, config, savedir = run_als_tucker_experiment_3d(
                ...
                resolution=40,
                random_seed=42,
                outdir="als_tucker3d_zulip_chebyshev_results",
            )
    ...
    with open("als_tucker3d_zulip_chebyshev_results.json", "w") as f_out:
